...
export default defineType({
  ...
  type: 'document',
  fields: [
    defineField({ name: 'titleNl', title: 'Title NL', type: 'string', validation: (Rule: any) => Rule.required() }),
    defineField({ name: 'titleEn', title: 'Title EN', type: 'string' }),
    defineField({ name: 'slug', title: 'Slug', type: 'slug', options: { source: 'titleNl' }, validation: (Rule: any) => Rule.required() }),
    ...
    defineField({ name: 'photos', title: 'Photos', type: 'array', of: [{ type: 'object', fields: [
      defineField({ name: 'image', title: 'Image', type: 'image', options: { hotspot: true }, validation: (Rule: any) => Rule.required() }),
      defineField({ name: 'altNl', title: 'Alt text NL', type: 'string' }),
      defineField({ name: 'altEn', title: 'Alt text EN', type: 'string' }),
    ], preview: { select: { title: 'altNl', media: 'image' }, prepare: ({ title, media }: { title?: string; media?: unknown }) => ({ title: title || 'Album photo', media }) } }] }),
