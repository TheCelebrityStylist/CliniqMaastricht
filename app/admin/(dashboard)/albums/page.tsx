...
export default async function AlbumsAdminPage({ searchParams }: { searchParams?: Promise<AlbumSearchParams> }) {
  ...
  return <div className="grid gap-8 xl:grid-cols-[1fr_480px]">
    <section>
      ...
      {!process.env.BLOB_READ_WRITE_TOKEN ? <p className="mt-5 rounded-2xl bg-yellow-50 p-4 text-sm font-bold text-yellow-800">Uploads are not active yet. Add BLOB_READ_WRITE_TOKEN in Vercel → Project Settings → Environment Variables. After redeploy, drag-and-drop upload will work.</p> : null}
    ...
    <aside className="rounded-[2rem] bg-white p-6 shadow-sm"><h2 className="text-2xl font-black">{edit ? 'Edit album' : 'Create album'}</h2><p className="mt-2 text-sm text-black/60">Step 1: album info. Step 2: bulk upload photos. Step 3: choose a cover. Step 4: publish.</p>
      <form action={saveAlbumAction} className="mt-6 grid gap-4"><input type="hidden" name="id" defaultValue={edit?.id}/>
        ...
        <MediaUploadField disabled={!hasBlob} label="Step 2: drag/drop 100+ album photos" />
