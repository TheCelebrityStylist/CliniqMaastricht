...
export default async function EventsAdminPage({ searchParams }: { searchParams?: Promise<{ edit?: string; duplicate?: string }> }) {
  ...
  return <div className="grid gap-8 xl:grid-cols-[1fr_500px]">
    <section>
      <p className="text-xs font-black uppercase tracking-[0.1em] text-[#f02688]">Agenda</p><h1 className="mt-3 text-5xl font-black tracking-[-0.03em]">Events Manager</h1>
    ...
    <aside className="rounded-[2rem] bg-white p-6 shadow-sm"><h2 className="text-2xl font-black">{query?.duplicate ? 'Duplicate event' : edit?._id ? 'Edit event' : 'Add event'}</h2><p className="mt-2 text-sm text-black/60">Required fields are grouped for a sub-2-minute publish workflow.</p>
      <form action={saveEventAction} className="mt-6 grid gap-4"><input type="hidden" name="_id" defaultValue={edit?._id} />
        ...
        <label className="grid gap-2 text-sm font-bold">Related photo album<select name="relatedAlbumId" defaultValue={edit?.relatedAlbumId || ''} className="rounded-2xl border border-black/10 bg-white px-4 py-3"><option value="">No album connected</option>{store.albums.map((album)=><option key={album.id} value={album.id}>{album.titleNl}</option>)}</select></label>
        ...
        <label className="grid gap-2 text-sm font-bold">Event type<select name="eventType" defaultValue={edit?.eventType || 'regular'} className="rounded-2xl border border-black/10 bg-white px-4 py-3"><option value="regular">Regular club night</option><option value="featured">Featured event</option><option value="special">Private/special event</option></select></label>
        <label className="flex items-center gap-3 rounded-2xl bg-[#f7f1e7] p-4 font-bold"><input type="checkbox" name="showDetailCTA" defaultChecked={edit?.showDetailCTA} /> Show detail page / CTA</label>
        ...
        <button className="rounded-full bg-[#f02688] px-5 py-3 text-sm font-black uppercase tracking-[0.1em] text-white">Save event</button>
      </form>
    </aside>
  </div>
}
