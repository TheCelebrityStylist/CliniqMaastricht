...
export default async function WorkshopPage() {
  ...
  const gallery = imageSets.workshop.map((url) => ({ url, alt: 'Cocktail workshop bij CLINIQ Maastricht', focalPoint: 'center' }))
  return <>
    <section className="relative min-h-[78vh] overflow-hidden pt-36"><SafeImage src={images.workshopBar} fallbackSrc={images.fallbackWide} alt="Gasten maken cocktails achter de bar bij CLINIQ Maastricht" fill priority sizes="100vw" className="-z-10 object-cover brightness-[1.1]" /><div className="absolute inset-0 -z-10 bg-gradient-to-r from-black via-black/56 to-transparent"/><div className="container-premium py-24"><h1 className="h1 max-w-5xl">{content?.heroTitle || 'Cocktail workshop bij CLINIQ'}</h1><p className="mt-7 max-w-2xl text-xl leading-8 text-white/76">{content?.heroSubtitle || 'Geen standaard borrel, maar samen achter de bar. Tijdens de cocktail workshop maak je meerdere cocktails onder begeleiding van onze bartenders. Ideaal voor vrijgezellenfeesten, bedrijfsuitjes, verjaardagen en vriendengroepen.'}</p><Link href="#form" className="btn-primary mt-8">Workshop aanvragen</Link></div></section>
