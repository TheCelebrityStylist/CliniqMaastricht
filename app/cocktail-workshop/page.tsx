...
export default async function WorkshopPage() {
  ...
  return <>
    <section className="relative min-h-[78vh] overflow-hidden pt-36"><SafeImage src={heroImage} fallbackSrc={images.fallbackWide} alt="Cocktail workshop Maastricht bij Cliniq" fill priority sizes="100vw" className="-z-10 object-cover brightness-[1.1]" objectPosition={gallery[0]?.focalPoint || 'center'} /><div className="absolute inset-0 -z-10 bg-gradient-to-r from-black via-black/65 to-transparent"/><div className="container-premium py-24"><p className="eyebrow">Cocktail workshop Maastricht</p><h1 className="h1 mt-5 max-w-4xl">{content?.heroTitle || 'Cocktail workshop Maastricht.'}</h1><p className="mt-7 max-w-2xl text-xl leading-8 text-white/75">{content?.heroSubtitle || 'Geen standaard groepsactiviteit, maar een smaakvolle start van je avond in Maastricht: shaken, proeven, lachen en leren van bartenders die weten hoe je een goede cocktail én een goede sfeer bouwt.'}</p><Link href="#form" className="btn-primary mt-8">Aanvraag doen</Link></div></section>
