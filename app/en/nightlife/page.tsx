...
export default async function NightlifePageEn() {
  ...
  return <>
    ...
    <section className="container-premium grid gap-8 pb-24 lg:grid-cols-[.85fr_1.15fr]"><div><p className="eyebrow">Practical</p><h2 className="h2 mt-4">Good to know</h2><div className="mt-8 grid gap-4 text-white/72 sm:grid-cols-2"><p>Thursday, Friday and Saturday.</p><p>Check the minimum age per event.</p><p>Always bring a valid ID.</p><p>Lockers are handled through /lockers.</p></div></div><div className="seo-panel rounded-[2rem] border border-white/10 bg-white/[0.045] p-7 md:p-9"><h2 className="h3">Club Maastricht, agenda and nightlife</h2><p className="mt-5 text-lg leading-8 text-white/70">CLINIQ is a club and cocktail bar in Maastricht for people looking for a night out beyond the standard bar setting. The venue is located on Platielstraat, within walking distance of Vrijthof, the Markt and the main nightlife streets. The agenda shows upcoming club nights, special events and themed nights. Minimum age can differ per event, so always check the event details before visiting.</p></div></section>
