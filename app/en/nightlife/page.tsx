...
export default async function NightlifePage() {
  ...
  return <section className="container-premium pt-36 pb-24">
    ...
    <p className="prose-premium mt-7 max-w-3xl">Cliniq brings a more polished way to go out in Maastricht: curated club nights, sharp cocktails, warm hospitality and a room designed for energy without losing style.</p>
    {events.length ? <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{events.map((event) => <EventCard key={event._id} event={event} />)}</div> : <div className="card mt-12 rounded-[2rem] p-8"><h2 className="h3">New events soon.</h2><p className="mt-3 text-white/70">There are no published upcoming events right now. Contact us for private events or check back soon.</p></div>}
