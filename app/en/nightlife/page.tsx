...
export default async function NightlifePageEn() {
  ...
  return <>
    ...
    <section className="event-section section-y"><div className="container-premium"><div className="reveal-up flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="eyebrow">Agenda</p><h2 className="h2 mt-3">This week at CLINIQ</h2><p className="mt-4 max-w-3xl text-lg leading-[1.65] text-white/70 md:text-xl">Next nights on Platielstraat.</p></div><Link className="btn-secondary hidden shrink-0 sm:inline-flex" href="/en/nightlife">All events</Link></div>{remainingEvents.length ? <div className={`event-grid event-grid-${Math.min(remainingEvents.length, 3)} mt-10`}>{remainingEvents.map((event, index) => <EventCard key={event._id} event={event} lang="en" priority={index === 0 && !featuredEvent} />)}</div> : !events.length ? <div className="mt-10 rounded-[2rem] border border-white/10 p-8 text-white/70">New events will be added soon.</div> : null}</div></section>
