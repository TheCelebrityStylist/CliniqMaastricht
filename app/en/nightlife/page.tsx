...
export default async function NightlifePageEn() {
  ...
  // Featured events (portrait promo flyers) get their own section, not this landscape grid - see
  // FeaturedEventsSection.tsx for why the two can't share one grid.
  ...
  const eventSchemas = events.map((event) => ({
    ...
    performer: { '@type': 'PerformingGroup', name: event.titleEn || event.title },
  ...
  return <>
    ...
    <FeaturedEventsSection events={events} lang="en" />
    <section id="agenda" className="event-section section-y"><div className="container-premium"><div className="reveal-up flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="eyebrow">Agenda</p><h2 className="h2 mt-3">This week at CLINIQ</h2><p className="mt-4 max-w-3xl text-lg leading-[1.65] text-white/70 md:text-xl">Next nights on Platielstraat.</p></div><Link className="btn-secondary hidden shrink-0 sm:inline-flex" href="/en/nightlife">All events</Link></div>{regularEvents.length ? <div className={`event-grid event-grid-${Math.min(regularEvents.length, 3)} mt-10`}>{regularEvents.map((event, index) => <EventCard key={event._id} event={event} lang="en" priority={index === 0} />)}</div> : <div className="mt-10 rounded-[2rem] border border-white/10 p-8 text-white/70">New events will be added soon.</div>}</div></section>
    {pilePhotos.length ? <section className="container-premium section-y"><p className="eyebrow">Last Saturday</p><h2 className="h2 mt-4">Drag the photos</h2><div className="mt-8"><PhotoPile photos={pilePhotos} lang="en" /></div></section> : null}
    <section className="container-premium section-y"><div className="grid auto-rows-[180px] gap-4 md:grid-cols-6 md:auto-rows-[240px]">{photos.map((src, index)=><div key={src} className={`photo-tile image-frame ${index === 0 || index === 3 ? 'md:col-span-2 md:row-span-2' : 'md:col-span-2'}`}><SafeImage src={src} fallbackSrc={images.fallbackWide} alt={`Nightlife Maastricht atmosphere ${index + 1}`} fill sizes="33vw" className="object-cover brightness-[1.08]" /></div>)}</div></section>
    <section className="container-premium section-y"><div className="mb-8"><p className="eyebrow">Photo albums</p><h2 className="h2 mt-3">Recent nights</h2></div><AlbumGrid albums={albums.slice(0, 3)} lang="en" /></section>
    <section className="container-premium section-y"><p className="eyebrow">Practical</p><h2 className="h2 mt-4">Good to know</h2><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><Practical title="Opening hours" text="CLINIQ is usually open on Thursday, Friday and Saturday. Check current times per event." /><Practical title="Minimum age" text="Age limits can differ per night. Always bring valid ID." /><Practical title="Door policy" text="We look at atmosphere, safety and respect. Arrive on time and well-presented." /><Practical title="Lockers" text="Lockers are handled through the official locker link on the website." /><Practical title="Location" text="Platielstraat 9A, within walking distance of Vrijthof, Markt and nightlife streets." /></div></section>
    <section className="container-premium section-y"><div className="seo-panel grid gap-8 rounded-[2rem] border border-white/10 bg-white/[0.045] p-7 md:p-10 lg:grid-cols-[.8fr_1.2fr]"><div><p className="eyebrow">Maastricht</p><h2 className="h2 mt-4">Nightlife in central Maastricht</h2></div><div className="prose-premium"><ChoreographedContent
      ...
      paragraphs={[
        ...SEO_TEXT.uitgaan.en.split('\n\n').map((paragraph) => <>{paragraph}</>),
        <><Link href="/en/cocktail-workshop" className="text-coral-text hover:text-white">Cocktail workshop Maastricht</Link> · <Link href="/en/event-space" className="text-coral-text hover:text-white">venue hire Maastricht</Link> · <Link href="/en/photos" className="text-coral-text hover:text-white">photos</Link> · <Link href="/en/contact" className="text-coral-text hover:text-white">contact</Link></>,
      ]}
    /></div></div></section>
    <section className="container-premium section-y"><p className="eyebrow">FAQ</p><h2 className="h2 mt-4">Frequently asked questions</h2><div className="faq-grid mt-8 grid gap-4 lg:grid-cols-2">{faqs.map((f)=><details key={f.question} className="luxury-panel rounded-2xl p-5"><summary className="cursor-pointer font-black">{f.question}</summary><p className="mt-3 text-white/70">{f.answer}</p></details>)}</div></section><JsonLd data={faqSchema(faqs)} /><JsonLd data={{ '@context': 'https://schema.org', '@graph': eventSchemas }} />
