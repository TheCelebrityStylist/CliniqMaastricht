...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const title = `${event.titleEn || event.title} | Nightlife Maastricht — Cliniq`
  const description = event.shortDescriptionEn || event.shortDescription || `${event.titleEn || event.title} at Cliniq Maastricht, Platielstraat 9A. Check times and tickets.`
  return {
    title,
    description,
    alternates: {
