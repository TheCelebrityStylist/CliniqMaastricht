...
export default async function UitgaanPage() {
  ...
  return <section className="container-premium pt-36 pb-24">
    <p className="eyebrow">Agenda · Uitgaan Maastricht</p><h1 className="h1 mt-5 max-w-5xl">{content?.heroTitle || 'Uitgaan bij CLINIQ'}</h1>
    <p className="prose-premium mt-7 max-w-3xl">{content?.heroSubtitle || 'Elke donderdag, vrijdag en zaterdag verandert CLINIQ in een clubavond met cocktails, DJ’s en een volwassen sfeer midden in Maastricht. Check de agenda, kom op tijd en neem je ID mee.'}</p>
    ...
    <div className="mt-16 grid gap-6 lg:grid-cols-2"><div className="luxury-panel rounded-[2rem] p-8"><h2 className="h3">Uitgaan in Maastricht.</h2><p className="mt-4 text-white/70">CLINIQ combineert cocktailbar, club en eventlocatie in het centrum. De avonden blijven overzichtelijk, met goede muziek, een sterke bar en genoeg ruimte om te dansen.</p></div><div className="luxury-panel rounded-[2rem] p-8"><h2 className="h3">Privé of met groep?</h2><p className="mt-4 text-white/70">Combineer jouw avond met een cocktail workshop of huur de event space voor een bedrijfsfeest, verjaardag of private party.</p><div className="mt-6 flex flex-wrap gap-3"><Link className="btn-primary" href="/cocktail-workshop">Workshop</Link><Link className="btn-secondary" href="/event-space">Event space</Link></div></div></div>
