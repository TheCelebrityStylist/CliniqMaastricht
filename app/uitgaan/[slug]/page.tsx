...
export default async function EventDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const event = await getAgendaEventBySlug(slug)
  if (!event) notFound()
  const title = 'nl' === 'en' ? event.titleEn || event.title : event.titleNl || event.title
  const subtitle = 'nl' === 'en' ? event.subtitleEn || event.subtitle : event.subtitleNl || event.subtitle
  const description = 'nl' === 'en' ? event.shortDescriptionEn || event.shortDescription : event.shortDescriptionNl || event.shortDescription
  return <section className="container-premium pt-36 pb-24">
    ...
    <div className="mt-8 grid gap-10 lg:grid-cols-[.9fr_1.1fr]">
      ...
      <div><p className="eyebrow">{event.date} · {event.startTime || '22:00'} · {event.ageLimit || '21+'}</p><h1 className="h1 mt-5">{title}</h1><p className="prose-premium mt-7">{description}</p>{event.ticketUrl ? <Link href={event.ticketUrl} target="_blank" className="btn-primary mt-8">Tickets / RSVP</Link> : null}</div>
    </div>
