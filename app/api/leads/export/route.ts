...
export async function GET(request: Request) {
  ...
  const rows = [['date','type','status','name','email','phone','source','message'], ...store.leads.map((lead) => [lead.createdAt, lead.formType, lead.status, lead.name, lead.email, lead.phone || '', lead.sourcePage || '', lead.message])]
