...
export default async function EventSpacePage(){
  ...
  const gallery = imageSets.eventSpace.map((url) => ({ url, alt: 'Ruimte huren bij CLINIQ Maastricht' }))
  return <>
    <section className="relative min-h-[78vh] overflow-hidden pt-36"><SafeImage src={images.redRoom} fallbackSrc={images.fallbackWide} alt="CLINIQ bar en ruimte met rode verlichting" fill priority sizes="100vw" className="-z-10 object-cover brightness-[1.08]" /><div className="absolute inset-0 -z-10 bg-gradient-to-r from-black via-black/62 to-burgundy/10"/><div className="container-premium py-24"><h1 className="h1 max-w-5xl">{content?.heroTitle || 'Ruimte huren bij CLINIQ'}</h1><p className="mt-7 max-w-2xl text-xl leading-8 text-white/76">{content?.heroSubtitle || 'Zoek je een feestlocatie in Maastricht die niet aanvoelt als een standaard zaal? CLINIQ is geschikt voor bedrijfsfeesten, verjaardagen, borrels, vrijgezellenfeesten en private events met bar, licht, geluid en clubgevoel.'}</p><a href="#aanvraag" className="btn-primary mt-8">Aanvraag doen</a></div></section>
