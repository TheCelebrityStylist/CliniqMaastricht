  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-ink/80 backdrop-blur-xl">
    <div className="container-premium flex h-20 items-center justify-between gap-6">
      ...
      <nav className="hidden items-center gap-7 lg:flex" aria-label="Hoofdnavigatie">
        {nav.map(([label, href]) => <Link key={href} href={href} className="text-xs font-extrabold uppercase tracking-[0.18em] text-white/70 transition hover:text-white">{label}</Link>)}
      </nav>
