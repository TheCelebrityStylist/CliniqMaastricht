...
// Regular weekly agenda card - landscape photo, own title + DJ line overlaid on the image. This
// is deliberately plain: featured events (which use a hand-designed portrait promo flyer with the
// title/lineup/deal already baked into the artwork) never render here any more - overlaying this
// card's own title on top of a flyer produced duplicated, ghosted text and flyer-copy
// bleed-through. They get their own FeaturedEventCard/FeaturedEventsSection instead (see
// components/ui/FeaturedEventCard.tsx), so every card this component renders links to the plain
// agenda anchor, never a generated detail page.
export function EventCard({ event, lang = 'nl', priority = false }: { event: EventWithAlbum; lang?: 'nl' | 'en'; priority?: boolean }) {
  ...
  return <>
    {/* Below md: a compact ~76px tappable row (date block, name/time, age+chevron) so several fit
       on screen at once - not a wall of full-height cards. The full card (below) is hidden here,
       shown from md up, so desktop is untouched by any of this. */}
    <Link
      href={agendaAnchor}
    ...
    <article
      ...
      className={`event-card group hidden md:block ${hasDetail ? 'event-card-featured' : 'event-card-regular'}`}
    >
      <Link href={agendaAnchor} className="block" data-track="agenda_card_click" aria-label={`${title} ${secondaryLine}`}>{media}</Link>
