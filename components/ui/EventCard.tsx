...
export function EventCard({ event }: { event: AgendaEvent }) {
  const date = new Date(`${event.date}T${event.startTime || '22:00'}`)
  return <article className="card group overflow-hidden rounded-[2rem]">
    <div className="relative aspect-[4/3] overflow-hidden bg-burgundy">
      <Image src={event.imageUrl || images.club} alt={event.imageAlt || `${event.title} bij Cliniq Maastricht`} fill sizes="(min-width:1024px) 33vw, 100vw" className="object-cover transition duration-700 group-hover:scale-105" />
