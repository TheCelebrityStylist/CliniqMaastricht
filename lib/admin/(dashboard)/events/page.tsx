...
export default async function EventsAdminPage({
  ...
  const formImage = formEvent ? resolveEventImage(formEvent) : null
    ...
    <div className="grid gap-8">
      <section>
        ...
        <p className="mt-3 max-w-3xl text-black/60">
          Create, edit, duplicate and publish agenda items directly inside the website.
        </p>

        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href="/admin/events"
            className="rounded-full bg-[#12070c] px-5 py-3 text-xs font-black uppercase tracking-widest text-white"
          >
            Add Event
          </Link>
          <Link
            href="/admin/bulk-events"
            className="rounded-full border border-black/10 bg-white px-5 py-3 text-xs font-black uppercase tracking-widest text-black/70"
          >
            Bulk Events
          </Link>
        </div>
        ...
        {!hasBlob ? (
          <p className="mt-4 rounded-2xl bg-yellow-50 p-4 text-sm font-bold text-yellow-800">
            Uploads are not active. Add BLOB_READ_WRITE_TOKEN in Vercel if you still want to use the old custom admin uploads.
          </p>
        ) : null}
      </section>
      ...
      <section className="rounded-[2rem] bg-white p-6 shadow-sm lg:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-widest text-[#f02688]">
              {editing ? 'Edit event' : duplicated ? 'Duplicate event' : 'Add event'}
            </p>
          ...
          <div className="flex flex-wrap gap-2">
            {presets.map((preset) => (
              <span
                key={preset.label}
                className="rounded-full bg-[#f5f1ea] px-4 py-2 text-xs font-black text-black/55"
              >
                {preset.label}: {preset.startTime}–{preset.endTime}, {preset.ageLimit}
              </span>
            ))}
          </div>
        </div>
        ...
        <form action={saveEventAction} className="mt-8 grid gap-6">
          ...
          <div className="grid gap-4 lg:grid-cols-4">
            ...
            <label className="grid gap-2 text-sm font-bold">
              ...
              <select
                name="eventType"
                defaultValue={formEvent?.eventType || 'regular'}
                className="rounded-2xl border border-black/10 px-4 py-3"
              >
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <label className="grid gap-2 text-sm font-bold">
              ...
              <input
                name="startTime"
                placeholder="22:00"
                defaultValue={formEvent?.startTime || ''}
                className="rounded-2xl border border-black/10 px-4 py-3"
              />
            </label>
            ...
            <label className="grid gap-2 text-sm font-bold">
              ...
              <input
                name="endTime"
                placeholder="03:00"
                defaultValue={formEvent?.endTime || ''}
                className="rounded-2xl border border-black/10 px-4 py-3"
              />
            </label>
            ...
            <label className="grid gap-2 text-sm font-bold">
              ...
              <input
                name="ageLimit"
                placeholder="21+"
                defaultValue={formEvent?.ageLimit || ''}
                className="rounded-2xl border border-black/10 px-4 py-3"
              />
            </label>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
            <div className="rounded-3xl border border-black/10 p-4">
              ...
              <p className="mt-1 text-xs text-black/50">
                This old admin image field is only used as fallback. New DJ/event images should be managed in Sanity.
              </p>
              ...
              {formEvent ? (
                <div className="mt-4 overflow-hidden rounded-2xl bg-black/5">
                  <div className="relative aspect-[4/3]">
                    <SafeImage
                      src={formImage?.imageUrl || images.fallbackEvent}
                  ...
                  <p className="p-3 text-xs font-black uppercase tracking-widest text-black/55">
                    Image source: {formImage?.imageSource || 'Fallback'}
                  </p>
                </div>
              ) : null}
              ...
              <div className="mt-4">
                <MediaUploadField
                  ...
                  multiple={false}
                  disabled={!hasBlob}
                  label="Upload event image override"
                />
              </div>
            </div>

            <details className="rounded-3xl border border-black/10 p-4">
              <summary className="cursor-pointer text-sm font-black">
                Featured/special event options
              </summary>

              <p className="mt-3 text-xs text-black/50">
                Regular nights ignore descriptions and CTAs on public cards.
              </p>

              <div className="mt-4 grid gap-3">
                <textarea
                  name="shortDescriptionNl"
                  defaultValue={formEvent?.shortDescriptionNl || ''}
                  placeholder="Short Dutch description"
                  className="min-h-24 rounded-2xl border border-black/10 px-4 py-3"
                />

                <textarea
                  name="shortDescriptionEn"
                  defaultValue={formEvent?.shortDescriptionEn || ''}
                ...
                <input
                  name="ticketUrl"
                  defaultValue={formEvent?.ticketUrl || ''}
                  placeholder="Ticket / CTA link"
                  className="rounded-2xl border border-black/10 px-4 py-3"
                />
              </div>
            </details>
          ...
          <div className="flex flex-wrap items-center gap-4 rounded-3xl bg-[#f5f1ea] p-4 text-sm font-bold">
            ...
            <label className="inline-flex items-center gap-2">
              <input
                type="checkbox"
                name="published"
                defaultChecked={formEvent?.published !== false}
              />
              Published
            </label>
            ...
            <label className="inline-flex items-center gap-2">
              <input
                type="checkbox"
                name="featured"
                defaultChecked={Boolean(formEvent?.featured)}
              />
              Featured
            </label>

            <input type="hidden" name="showDetailCTA" value="false" />
            <label className="inline-flex items-center gap-2">
              <input
                type="checkbox"
                name="showDetailCTA"
                defaultChecked={Boolean(formEvent?.showDetailCTA)}
              />
              Show detail page / CTA
            </label>
          </div>

          <button className="min-h-11 rounded-full bg-[#f02688] px-6 py-4 text-xs font-black uppercase tracking-widest text-white hover:bg-[#12070c]">
            Save Event
          </button>
        </form>
      </section>
                ...
                return (
                  <tr key={event._id} className="align-middle">
                    <td className="px-4 py-4">
                      <div>
                        <div className="relative h-16 w-12 overflow-hidden rounded-xl bg-black/5">
                          <SafeImage
                        ...
                        <p className="mt-1 text-[10px] font-black uppercase tracking-widest text-black/45">
                    ...
                    <td className="px-4 py-4">
                      ...
                      <div className="text-xs text-black/45">{event.slug?.current}</div>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Link
                          href={`/admin/events?duplicate=${event._id}`}
                          className="rounded-full border border-black/10 px-3 py-2 text-xs font-black"
                        >
                          Duplicate
                        </Link>
                        ...
                        <form action={toggleEventPublishedAction}>
                          ...
                          <button className="rounded-full border border-black/10 px-3 py-2 text-xs font-black">
                            {event.published === false ? 'Publish' : 'Unpublish'}
                          </button>
                        </form>

                        <form action={useDjImageAction}>
                          <input type="hidden" name="id" value={event._id} />
                          <button className="rounded-full border border-black/10 px-3 py-2 text-xs font-black">
                            Use DJ image
                          </button>
                        ...
                        <form action={deleteEventAction}>
                          <input type="hidden" name="id" value={event._id} />
                          <button className="rounded-full bg-red-50 px-3 py-2 text-xs font-black text-red-700">
                            Delete
                          </button>
                        </form>
                      </div>
